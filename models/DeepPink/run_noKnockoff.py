...
for dataType in dataTypeList:
    ...
    x_url = os.path.join(dataDir, dataType, 'X.csv'); print(x_url)
    ...
    for testIndices in testIndicesList:
        ...
        x3D_train[:, :, 0] = xvalues[trainIndices,:];
        x3D_train[:, :, 1] = xvalues[trainIndices,:];
        label_train = yvalues[trainIndices];

        x3D_test = np.zeros((len(testIndices), pVal, 2));
        x3D_test[:, :, 0] = xvalues[testIndices, :];
        x3D_test[:, :, 1] = xvalues[testIndices, :];
        label_test = yvalues[testIndices];

        coeff = 0.05*np.sqrt(2.0 * np.log(pVal) / x3D_train.shape[0]);
        outputDir = os.path.join(dataDir, dataType, 'result_2layer_epoch' + str(num_epochs) + '_batch' + str(batch_size) + '_knockoff0');
        try:
            os.stat(outputDir)
        except:
            os.mkdir(outputDir)

        rocVec = np.zeros(iterNum)
        for iter in range(iterNum):
            ...
            fpr, tpr, thresholds = roc_curve(y_true=label_test, y_score=DNN.predict(x3D_test));
            roc_auc = auc(x=fpr, y=tpr)
            rocVec[iter] = roc_auc;

            del DNN
        print(rocVec);
        allRoc = np.vstack((allRoc, rocVec.reshape((1, iterNum))));

    np.savetxt(os.path.join(outputDir, 'result_auroc.csv'), allRoc, fmt='%.6f', delimiter=",")
